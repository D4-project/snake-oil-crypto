...
class FeedGenerator:
    ...
    def save_manifest(self):
        try:
            manifestFile = open(os.path.join(self.confp['FEEDGENERATOR']['OUTPUT'], 'manifest.json'), 'w')
    ...
    def save_hashes(self):
        if len(self.attributeHashes) == 0:
            return False
        try:
            hashFile = open(os.path.join(self.confp['FEEDGENERATOR']['OUTPUT'], 'hashes.csv'), 'a')
    ...
    def get_last_event_from_manifest(self):
        ...
        """
        try:
            manifest_path = os.path.join(self.confp['FEEDGENERATOR']['OUTPUT'], 'manifest.json')
    ...
    def _get_event_from_id(self, event_uuid):
        with open(os.path.join(self.confp['FEEDGENERATOR']['OUTPUT'], '%s.json' % event_uuid), 'r') as f:
